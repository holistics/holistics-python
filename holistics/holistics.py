...
class HolisticsAPI:
    ...
    def get_export_results(self, job_id, _page_size = None, _page = None):
        ...
        if _page_size is not None:
            self.page['_page_size']=str(_page_size)
        if _page is not None:
            self.page['_page']=str(_page)
        try:
    ...
    def export_data(self,report_id, path=None,filters=None, page_size=None, page=None):
        try:
            ...
            self.get_export_results(self.page['job_id'],page_size,page)
            res = self.download_results(path)
        except (requests.exceptions.HTTPError, RuntimeError, pd.errors.ParserError):
